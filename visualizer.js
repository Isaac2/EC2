...
fs.readdir('./results/',function(err,files){
  files.forEach(element => {
    fs.readFile('./results/'+element, 'utf8', function(err,data){
      if(err){
        if(err.errno == 34){
          console.log("Cannot find the files- did you copy it from ");
          console.log("your 'output' directory to the current directory?");
          console.log("Try running 'cp ../results/ .'");
          process.exit(1);
        }
        console.log("Cannot read from '/results/<file>': "+err);
        process.exit(1);
      }
